// LifeSafe v1.15b-Light — self-contained (no modules).
// Goal: do NOT hang on splash; always show app even if Firebase fails.
...
(function boot(){
  ...
  // Always show within 800ms even if something goes wrong
  ...
  (function firebaseAuth(){
    ...
    try{
      ...
      auth.signInAnonymously().catch((err)=>{
        ...
        set2('Check Firebase Console: Authentication → Sign-in method → Anonymous enabled.');
      });
    }catch(err){
      set1('Firebase init error: ' + (err && err.message ? err.message : String(err)));
      set2('If this says SDK not loaded, the firebase script may be blocked/cached.');
    }
  })();

  // ---- App UI + local storage (same behaviour as earlier locked builds) ----
  (function app(){
    const tabs=[
    ...
    const tabsEl=byId('tabs'), titleEl=byId('activeTitle'), detail=byId('detail');
    const refs={
    ...
    // Keep your existing data (same key as v1.14a)
    ...
    function toggleHint(tab){
      const r=refs[tab]; if(!r) return;
      if(r.hint) r.hint.classList.toggle('hidden', (data[tab]||[]).length>0);
    }
    function banner(tab){
      if(tab==='uploads') return;
      const r=refs[tab], arr=data[tab]||[];
    ...
    function renderList(tab){
      ...
      if(tab==='uploads'){
        arr.forEach(rec=>{
          ...
          const badge=document.createElement('span'); badge.className='badge pending'; badge.textContent='Not synced';
      ...
      arr.forEach(rec=>{
        ...
        if(rec.renewalDate){
          ...
          ln.textContent='Renewal: '+fmtDate(rec.renewalDate)+(past(rec.renewalDate)?'  ❗':'');
          if(past(rec.renewalDate)) ln.classList.add('expired');
        ...
        g.onclick=(e)=>{ e.stopPropagation(); openG(rec); };
    ...
    function switchTab(id){
      active=id;
      tabs.forEach(t=>{
        byId(t.content).classList.add('hidden');
        byId('tab-'+t.id).classList.remove('active');
      });
      const t=tabs.find(x=>x.id===id);
      byId(t.content).classList.remove('hidden');
    ...
    function openDetail(tab,id){
      viewing={tab,id};
      const rec=(data[tab]||[]).find(r=>r.id===id);
      if(!rec) return;
      ...
      if(tab!=='uploads'){
        ...
        if(rec.renewalDate){
          const n=daysTo(rec.renewalDate);
          const ln=document.createElement('div'); ln.textContent='Renewal Date: '+fmtDate(rec.renewalDate);
      ...
      if(dates.childNodes.length) card.appendChild(dates);
      ...
      if(tab==='uploads'){
        calFromDetail.style.display='none';
        editFromDetail.style.display='none';
      }else{
        calFromDetail.style.display='';
        editFromDetail.style.display='';
        calFromDetail.onclick=()=>openG(rec);
    ...
    Object.keys(refs).forEach(k=>{
      const r=refs[k];
      r.addBtn.addEventListener('click', ()=>{
        if(k==='uploads'){
          r.file.value='';
          r.file.click();
        }else{
    ...
    refs.uploads.file.addEventListener('change', (e)=>{
      ...
      const items=files.map(f=>({
        ...
        modified: f.lastModified ? new Date(f.lastModified).toISOString() : now,
        createdAt: now
      }));
    ...
    function reallyRemove(tab,id){
      data[tab]=(data[tab]||[]).filter(r=>r.id!==id);
      save();
      renderList(tab);
    ...
    load();
    // show app quickly after load; splash is handled above too
    setTimeout(()=>{ try{ splash.classList.add('hidden'); main.classList.remove('hidden'); }catch(e){} }, 500);
    switchTab('home');
  })();
